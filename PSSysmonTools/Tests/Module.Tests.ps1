    ...
    foreach ($Function in $FunctionsList)
    {
        ...
        Context "Exported command parameters for $Function" {
            It 'should have have an OutputType attribute' {
                $OutputTypeAttribute = $AST.ParamBlock.Attributes | Where-Object { $_.TypeName.Name -eq 'OutputType' } | Select -First 1
        ...
        Context "Required exported command naming scheme for $Function" {
            It 'should have a "Sysmon" noun prefix' {
                $Module.ExportedCommands[$Function].Noun.Substring(0, 6) | Should BeExactly 'Sysmon'
            }
        }
        ...
        Context "Comment-based help for: $Function"{
            ...
            It 'should have an author listed' {
                $Help.Description.Text.Contains('Author:') | Should Be $True
            }

            It 'should contain a BSD license in the synopsis' {
                $Help.Description.Text.Contains('License: BSD 3-Clause') | Should Be $True
            }
